...
void CPushinPet::OnRender()
{
        ...
        else // WALKING — state machine + BFS + smooth physics
        {
                ...
                // State selection — with HYSTERESIS to prevent rapid switching.
                // Only change state if the new condition is significantly different
                // from the current one (prevents flicker between CHASE/DROP/CLIMB).
                {
                        int NewState = m_PetState; // default: keep current
                        if(m_StuckTimer > 0.5f)
                                NewState = PET_STUCK;
                        else if(m_PetState == PET_STUCK && m_StuckTimer < 0.1f)
                                NewState = PET_CHASE; // exit STUCK when unstuck
                        else if(m_PetState != PET_STUCK)
                        {
                                if(DistY > 50.0f && OnGround && Dist > 60.0f)
                                        NewState = PET_DROP;
                                else if(DistY < -40.0f && Dist > 60.0f)
                                        NewState = PET_CLIMB;
                                else if(std::abs(DistY) < 30.0f) // only go to CHASE if roughly level
                                        NewState = PET_CHASE;
                                // else: keep current state (hysteresis — don't switch for small changes)
                        }
                        m_PetState = NewState;
                }

                // === PHYSICS — very smooth acceleration ===
                const float MinDist = 50.0f;
                const float MaxSpeed = 550.0f;
                const float Accel = 3000.0f * Dt;  // slow acceleration = smooth
                const float Decel = 4000.0f * Dt;  // slightly faster deceleration
                ...
                if(DistX > MinDist)
                        ...
                        DesiredVelX = 0.0f; // just stop when close — NO running backward

                // DROP state: always run toward target, even off edges
                if(m_PetState == PET_DROP)
                        DesiredVelX = MoveDir * MaxSpeed;

                // STUCK state: don't run, just jump/hook
                if(m_PetState == PET_STUCK)
                        DesiredVelX = 0.0f;
