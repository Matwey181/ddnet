...
void CScoreboard::RenderSpectators(CUIRect Spectators)
{
        ...
        for(const CNetObj_PlayerInfo *pInfo : GameClient()->m_Snap.m_apInfoByName)
        {
                ...
                // Pushin client: apply [prefix] name + tint color on scoreboard.
                {
                        char aPushinName[64];
                        CMenus::GetPushinDisplayName(aPushinName, sizeof(aPushinName), pInfo->m_ClientId, GameClient()->m_aClients[pInfo->m_ClientId].m_aName);
                        const ColorRGBA PushinColor = CMenus::GetPushinNickColor(pInfo->m_ClientId);
                        if(PushinColor != ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f))
                                TextRender()->TextColor(PushinColor);
                        TextRender()->TextEx(&Cursor, aPushinName);
                        TextRender()->TextColor(TextRender()->DefaultTextColor());
                }
        ...
        for(int RenderDead = 0; RenderDead < 2; RenderDead++)
        {
                for(int i = 0; i < MAX_CLIENTS; i++)
                {
                        ...
                        else
                        {
                                ...
                                const int PushinEmote = CMenus::GetPushinEmote(pInfo->m_ClientId);
                                RenderTools()->RenderTee(CAnimState::GetIdle(), &TeeInfo, PushinEmote, vec2(1.0f, 0.0f), TeeRenderPos);
                        }
                        ...
                        // name
                        {
                                ...
                                // Pushin client: apply [prefix] name + tint color on scoreboard name.
                                {
                                        char aPushinName[64];
                                        CMenus::GetPushinDisplayName(aPushinName, sizeof(aPushinName), pInfo->m_ClientId, ClientData.m_aName);
                                        const ColorRGBA PushinColor = CMenus::GetPushinNickColor(pInfo->m_ClientId);
                                        if(PushinColor != ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f))
                                                TextRender()->TextColor(PushinColor);
                                        TextRender()->TextEx(&Cursor, aPushinName);
