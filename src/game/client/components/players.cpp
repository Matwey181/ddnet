        ...
        float Intra)
{
        ...
        // Pushin client — Var List: tint the skin and override emote for war/team players
        if(ClientId >= 0 && ClientId < MAX_CLIENTS)
        {
                int PushinEmoteOverride = Player.m_Emote;
                if(PushinApplyTintToPlayerSkin(GameClient()->m_aClients[ClientId].m_aName,
                        RenderInfo.m_ColorBody, RenderInfo.m_ColorFeet, RenderInfo.m_BloodColor,
                        PushinEmoteOverride))
                {
                        Player.m_Emote = PushinEmoteOverride;
                }
        }
