        ...
        float Intra)
{
        ...
        // Pushin client: override emote for var (angry) / team (happy) players.
        const int PushinEmote = CMenus::GetPushinEmote(ClientId);
        const int RenderEmote = (PushinEmote != EMOTE_NORMAL) ? PushinEmote : Player.m_Emote;
