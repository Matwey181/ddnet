...
void CNamePlates::RenderNamePlateGame(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha)
{
        ...
        // Pushin client: apply [prefix] name + tint color in-game.
        CMenus::GetPushinDisplayName(Data.m_aName, sizeof(Data.m_aName), pPlayerInfo->m_ClientId, GameClient()->m_aClients[pPlayerInfo->m_ClientId].m_aName);
        ...
        // Pushin client: override nick color for var/team players.
        {
                const ColorRGBA PushinColor = CMenus::GetPushinNickColor(pPlayerInfo->m_ClientId);
                if(PushinColor != ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f))
                        Data.m_Color = PushinColor;
        }
