...
function make_all_sqlite3() {
	if [[ "${TARGET_PLATFORM}" == "android" ]]; then
		...
		make_sqlite3 "${IOS_DEVICE_BUILD_FOLDER}" "" "${IOS_COMMON_CFLAGS} ${IOS_EXTRA_RELEASE_CFLAGS}" "iphoneos" "${IOS_DEVICE_ARCH}"
		make_sqlite3 "${IOS_SIM_ARM64_BUILD_FOLDER}" "" "${IOS_COMMON_CFLAGS} ${IOS_EXTRA_RELEASE_CFLAGS}" "iphonesimulator" "${IOS_SIM_ARM64_ARCH}"
		make_sqlite3 "${IOS_SIM_X64_BUILD_FOLDER}" "" "${IOS_COMMON_CFLAGS} ${IOS_EXTRA_RELEASE_CFLAGS}" "iphonesimulator" "${IOS_SIM_X64_ARCH}"
	else
